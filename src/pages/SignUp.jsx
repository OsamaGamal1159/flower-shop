...
function SignUp() {
  ...
  const validationForm = () => {
    ...
    } else if (!/^(01)\d{9}$/.test(values.mobile_number)) {
    ...
    if (!values.password) {
      ...
      !/^(?=.*[0-9])(?=.*[a-zA-Z])[a-zA-Z0-9]{4,10}$/i.test(values.password)
    ) {
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr] pb-20 pt-2 px-10 h-full">
      <div
        className="relative w-full h-full bg-cover bg-center bg-no-repeat bg-red-300"
      ...
      <div className="flex flex-col justify-center items-center px-20 text-gray-800">
        ...
        {authError && (
          <p className="w-full text-center text-red-500 font-medium bg-red-100 p-3 my-5">
            {authError}
          </p>
        )}
        <button
          ...
          className="w-full rounded-md bg-green-2 text-lg font-bold text-white py-3 mt-5"
        >
          Sign Up
        </button>
        <p className="pt-5">
